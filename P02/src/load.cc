 ...
 * @date: 18/02/2025
 * @brief implementacion de la clase Load
*/
 ...
 */
void Load::ejecutar(Contexto& contexto) {
  switch (direccionamiento_) {
    case Direccionamiento::DIRECTO:
      contexto.memoria_datos->setValor(contexto.memoria_datos->getValor(operando_));
      break;
    case Direccionamiento::INDIRECTO:
      contexto.memoria_datos->setValor(contexto.memoria_datos->getValor(contexto.memoria_datos->getValor(operando_)));
      break;
    case Direccionamiento::INMEDIATO:
      contexto.memoria_datos->setValor(operando_);
      break;
  }
...
/**
 * @brief parsea la instrucción load
 * @param instruccion contiene la instrucción a parsear
 */
void Load::parsear(std::string& instruccion) {
