 ...
 * @date: 18/02/2025
 * @brief implementacion de la clase Div
*/
 ...
 */
void Div::ejecutar(Contexto& contexto) {
  switch (direccionamiento_) {
    case Direccionamiento::DIRECTO:
      if (contexto.memoria_datos->getValor(operando_) == 0) {
    ...
    case Direccionamiento::INDIRECTO:
      if (contexto.memoria_datos->getValor(contexto.memoria_datos->getValor(operando_)) == 0) {
        std::cerr << "Error: División por cero" << std::endl;
        throw('e');
      }
      contexto.memoria_datos->setValor(contexto.memoria_datos->getValor(0) / 
...
/**
 * @brief parsea la instrucción div
 * @param instruccion contiene la instrucción a parsear
 */
void Div::parsear(std::string& instruccion) {
