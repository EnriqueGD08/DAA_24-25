 ...
 * @date: 22/04/2025
 * @brief: Implementación de la clase GVNS
*/
...
/**
 * @brief Resuelve el problema utilizando el algoritmo GVNS.
 * @return void
 */
void GVNS::resolver() {
  ...
  while (k_ < max_k_) {
    for (int i = 0; i < iteraciones_construccion_; i++) { // construcción de GRASP
      resetear_solucion();
      construccion();
      soluciones_.push_back(solucion_);
    }
    mejor_solucion = busquedas_locales(); // VND
    if (mejor_solucion.get_subrutas() < solucion_.get_subrutas()) {
      ...
      // Resetear la solución para la siguiente iteración
      soluciones_.clear(); 
      resetear_solucion();
    }
    k_++;
 ...
 * @return void
 */
Solucion GVNS::busquedas_locales() {
  Solucion mejor_solucion;
  ...
  for (Solucion& solucion : soluciones_) {
    ...
    // std::vector<int> indice_busquedas = {0, 1, 2, 3};

    while (limite < 4) { // (indice_busquedas.size() > 0) {
      Solucion auxiliar = solucion;
      // std::random_shuffle(indice_busquedas.begin(), indice_busquedas.end());
      // limite = indice_busquedas[0];
      switch (limite) {
      case 0:
        for (int indice_ruta = 0; indice_ruta < cantidad_subrutas; indice_ruta++) { //busquedas locales dentro de la misma ruta
          busqueda_local1(solucion, indice_ruta);
        }
        if (solucion.get_subrutas() < auxiliar.get_subrutas()) {
          auxiliar = solucion;
          // indice_busquedas = {1, 2, 3};
        } else {
          solucion = auxiliar;
        }
        limite++;
        break;
      case 1:
        for (int indice_ruta = 0; indice_ruta < cantidad_subrutas; indice_ruta++) {
          for (int indice_subruta = 0; indice_subruta < cantidad_subrutas; indice_subruta++) { //busquedas locales entre rutas
            if (indice_ruta != indice_subruta) {
              busqueda_local2(solucion, indice_ruta, indice_subruta);
            }
          }
        }
        if (solucion.get_subrutas() < auxiliar.get_subrutas()) {
          auxiliar = solucion;
          // indice_busquedas = {0, 2, 3};
        } else {
          solucion = auxiliar;
        }
        limite++;
      ...
      case 2:
        for (int indice_ruta = 0; indice_ruta < cantidad_subrutas; indice_ruta++) { //busquedas locales dentro de la misma ruta
          busqueda_local3(solucion, indice_ruta);
        }
        if (solucion.get_subrutas() < auxiliar.get_subrutas()) {
          auxiliar = solucion;
          // indice_busquedas = {0, 1, 3};
        } else {
          solucion = auxiliar;
        }
        limite++;
      ...
      case 3:
        for (int indice_ruta = 0; indice_ruta < cantidad_subrutas; indice_ruta++) {
          for (int indice_subruta = 0; indice_subruta < cantidad_subrutas; indice_subruta++) { //busquedas locales entre rutas
            if (indice_ruta != indice_subruta) {
              busqueda_local4(solucion, indice_ruta, indice_subruta);
            }
          }
        }
        if (solucion.get_subrutas() < auxiliar.get_subrutas()) {
          auxiliar = solucion;
          // indice_busquedas = {0, 1, 2};
        } else {
          solucion = auxiliar;
        }
        limite++;
 ...
 * @return void
 */
Solucion& GVNS::busqueda_local1(Solucion& solucion, int indice_ruta) { // Intercambio de nodos en la misma ruta
 ...
 * @return void
 */
Solucion& GVNS::busqueda_local2(Solucion& solucion, int indice_subruta1, int indice_subruta2) { // Intercambio de entre dos nodos de diferentes rutas
 ...
 * @return void
 */
Solucion& GVNS::busqueda_local3(Solucion& solucion, int indice_ruta) { // Inserción de un nodo de una subruta a otra subruta (no intercambio)
  std::vector<Nodo> ruta = solucion.get_nodos()[indice_ruta];
 ...
 * @return void
 */
Solucion& GVNS::busqueda_local4(Solucion& solucion, int indice_ruta1, int indice_ruta2) { // Inserción de un nodo de una ruta a otra ruta (no intercambio)
