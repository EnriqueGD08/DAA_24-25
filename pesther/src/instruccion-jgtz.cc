 ...
 * @date 14 Feb 2024
 * @brief Implementación de la clase "InstruccionJGTZ"
...
void InstruccionJGTZ::ejecutar(Contexto& contexto) {
  #ifdef DEPURAR
  std::cout << "Ejecutando instrucción JGTZ..." << std::endl;
  #endif
  if (contexto.memoria_datos->get_valor(0) > 0) {
    if (contexto.etiquetas.find(etiqueta_) == contexto.etiquetas.end()) {
      ...
      std::cerr << "--> void InstruccionJGTZ::ejecutar(Contexto& contexto)" << std::endl;
      std::cerr << "--> Línea 29 de instruccion-jgtz.cc" << std::endl;
      exit(1);
    }
...
void InstruccionJGTZ::parsear(std::string& instruccion) {
  #ifdef DEPURAR
  std::cout << "Parseando instrucción JGTZ..." << std::endl;
  #endif
 ...
 */
std::ostream& InstruccionJGTZ::write(std::ostream& os, bool debug) const {
  if (debug) {
    os << "\nOpcode: " << opcode_;
