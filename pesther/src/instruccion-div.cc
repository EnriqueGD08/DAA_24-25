 ...
 * @date 14 Feb 2024
 * @brief Implementación de la clase "InstruccionDIV"
...
void InstruccionDIV::ejecutar(Contexto& contexto) {
  #ifdef DEPURAR
  std::cout << "Ejecutando instrucción DIV..." << std::endl;
  #endif
  switch (direccionamiento_) {
    case Direccionamiento::DIRECTO:
      ...
      if (contexto.memoria_datos->get_valor(operando_) == 0) {
        std::cerr << RED "Error: División por cero" NC << std::endl;
        std::cerr << "--> void InstruccionDIV::ejecutar(Contexto& contexto)" << std::endl;
        std::cerr << "--> Línea 32" << std::endl;
        exit(1);
      }
    ...
    case Direccionamiento::INDIRECTO:
      ...
      if (contexto.memoria_datos->get_valor(contexto.memoria_datos->get_valor(operando_)) == 0) {
        std::cerr << RED "Error: División por cero" NC << std::endl;
        std::cerr << "--> void InstruccionDIV::ejecutar(Contexto& contexto)" << std::endl;
        std::cerr << "--> Línea 52 de instruccion-div.cc" << std::endl;
        exit(1);
      }
...
void InstruccionDIV::parsear(std::string& instruccion) {
  #ifdef DEPURAR
  std::cout << "Parseando instrucción DIV..." << std::endl;
  #endif
 ...
 */
std::ostream& InstruccionDIV::write(std::ostream& os, bool debug) const {
  if (debug) {
    os << "\nOpcode: " << opcode_;
